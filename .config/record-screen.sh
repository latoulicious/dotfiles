...
if pgrep -x wf-recorder >/dev/null; then
    ...
    log_msg "Starting screen recording to: $OUT"
    
    if command -v notify-send >/dev/null; then
        notify-send "Screen Recorder" "Recording SCREEN… Press hotkey again to stop."
    fi
    ...
    /usr/bin/wf-recorder $AUDIO_ARG -f "$OUT" >>"$LOG" 2>&1 &
    WF_PID=$!
